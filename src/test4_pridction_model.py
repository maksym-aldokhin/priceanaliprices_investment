...
positive_data = []
negative_data = []
with open(os.getcwd() + '/training/train.csv', mode ='r', encoding='utf-8') as file:
    ...
    for row in csvreader:
        if int(row[0]) == 1:
            ...
            labels.append(1)
            positive_data.append(row[2])
with open(os.getcwd() + '/training/test.csv', mode ='r', encoding='utf-8') as file:
    csvreader = csv.reader(file, delimiter = ',')
    for row in csvreader:
        if int(row[0]) == 1:
            labels.append(-1)
...
labels = ['positive'] * len(positive_data) + ['negative'] * len(negative_data)
...
print(real_data, "   Predicted sentiment:", predicted_sentiment)
